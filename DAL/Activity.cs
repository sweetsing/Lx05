...
namespace DAL
{
   public class Activity
    {
        private Activity() { }
        public static Activity _instance = new Activity();
        ...
        public int GetVerifyCount()
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql="select count(1) from activity where activtyVerify='审核通过'";
        ...
        public IEnumerable<Model.ActivityNo> GetPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by endTime dese) as num,activity.*from activity)";
                sql += "select* from a where num between(@pageIndex-1)*@pageSize+1 and @pageIndex*@pageSize;";
        ...
        public int UpdateRecommend(Model.Activity activity)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "update activity set recommend=@recommend,recommendTime=@recommendTime where activity=@activityId";
